            ...
            options.controllerPath.forEach((dir) => {
                fs.readdirSync(dir).forEach((file: string) => {
                    ...
                    if (file.indexOf(ext) !== -1) {
                        ...
                        this.pathObjectTab = [];
                        console.log('RONININININI', this.depthOf(controllerCurrent, this.pathObjectTab), controllerCurrent )
                        const pathObjectString = this.depthOf(controllerCurrent, this.pathObjectTab).join('.');
                        const finalPath = pathObjectString.startsWith('.') ? pathObjectString.slice(1) : pathObjectString;

                        this.controllers[finalPath + '.' + name] = controllerCurrent;
                    }
                });
            });
